...
static DEFINE_MUTEX(orc_sort_mutex);

#define orc_sort_mutex_lock()	mutex_lock(&orc_sort_mutex)
#define orc_sort_mutex_unlock()	mutex_unlock(&orc_sort_mutex)
#else /* ORC_COMPRESSED_BOOT */
#define orc_sort_mutex_lock()
#define orc_sort_mutex_unlock()
...
void orc_sort(int *ip_table, struct orc_entry *orc_table, u32 num_orcs)
{
	...
	sort(ip_table, num_orcs, sizeof(*ip_table), orc_sort_cmp,
	     orc_sort_swap);

	orc_sort_mutex_unlock();
}
